...
\begin{abstract}
	...
	% tendon-driven robotic fingers with non-elastic bifurcate tendons to
	...
	% したものである．
	% In particular, we focus on the non-elastic bifurcate tendon that
	...
	bifurcate tendon when no external force is exerted.
	% In this robotic finger, the displacement angles of the two joins are
	% coupled by the non-elastic bifurcate tendon in the situation where no
	% external force is exerted.
	% 分岐腱が非伸縮な場合，フィンガは，外力がない状態では，幾何学的な拘束に
	% より関節が連動して1自由度で動作する．
	If a certain amount of external force is applied on the fingertip,
	one of tendons slacks and the coupling between two joints will be lost.
	...
	% 緩みが生じ，関節の連動が生じなくなる．
	This means that the two-DoF robotic finger is easily controlled as one
	...
	Based on this idea, we develop and control the two-DoF robotic finger
	that equipes two tendons including the non-elastic bifurcate tendon.
