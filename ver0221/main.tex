...
\begin{abstract}
	...
	tendon-driven robotic fingers with nonelastic bifurcate tendons to
	...
	% 緩みが生じ，関節の連動が生じなくなる．
	This means that the two DOFs robotic finger is easily controlled as one
	...
	Based on this idea, we develop and control the two DOFs robotic finger
...
% &\multicolumn{3}{@{}l}{Tiergartenstraﾟe 17}\\
...
The finger has two DoFs and two tendons noted as tendon 1 and tendon 2.
...
In this case, align (\ref{jaco}) can be represented as follows:
\begin{align}
	\begin{bmatrix}
		\dot{l}_{1}\\
		\dot{l}_{2b}\\
...
align (\ref{jaco_relation}) can be divided as follows:
\begin{align}
...
Therefore, $\dot{l}_{2a}=\dot{l}_{2b}=\dot{l}_{2}$ and align (\ref{subjacobian1}) can be rewritten as:
\begin{align}
	\dot{l}_{2}
	\begin{bmatrix}
		1\\
...
We define a virtual angle $\theta_v$ to represent the angle of coupled joints as $\dot{\theta_v}=\dot{l_{1}}$.
Substituting align (\ref{theta}) into align (\ref{jaco_relation}), we obtain the follows:
\begin{align}
	\begin{bmatrix}
		\dot{l}_{1}\\
		\dot{l}_{2b}\\
...
\end{align}
From the fact that $\dot{l}_{2a}=\dot{l}_{2b}=\dot{l}_{2}$ when there is no slack, align (\ref{new_jaco_relation}) can be rewritten as:
\begin{align}
	\begin{bmatrix}
		\dot{l}_{1}\\
		\dot{l}_{2}
...
Second, we apply this matrix to align (\ref{force_relation}).
Multiplying both sides of align (\ref{force_relation}) by $(\vect{P}_{2}^{-1}[1\ 1]^{T})^{T}$, we obtain the follows:
\begin{align}
	\begin{bmatrix}
		1&1
	\end{bmatrix}
...
Assuming the frictionless point contact, the external force vector can be described as $\vect{F}=[0\ F]^{T}$, and align (\ref{standard_jacobian}) can be written as:
\begin{align}
	\vect{\tau}=
		\begin{bmatrix}
...
Therefore, substituting align (\ref{tau}) into align (\ref{tensile_force}), we obtain the follows:
\begin{align}
	\begin{bmatrix}
	f_{1}\\
	f_{2b}\\
...
% \section{robotic finger with nonelastic bifurcate tendon} % (fold)
...
Finger has two DOFs in flexion/extension and is composed of three links and five pulleys.
...
Also, the ratio between the changes in the joint angle vector shown by Eq.\ref{theta} is calculated as follows:
...
the rotation angle of a base pulley which wind the tendon, is measured by rotary encoder(OMRON E6A2-CW3C).
...
Arduino gives any instructions to servo motors, reads the value of counter, and calculate variables used for control of finger.
...
In order to make tendons of the finger slacken, either value of tensile force of tendon 1s or 1p which is calculated by Eq.\ref{eq:f-tau-pinv} have to become negative.
Using $\theta_1=2\theta_2$ and the tendon Jacobian $\bm{P}$ derived from the moment arms represented in Table \ref{tab:pulley}, 
% $\bm{A}$, $\bm{f}_b$ in 
Eq.\ref{eq:f-tau-pinv} is defined as follows:
...
\begin{figure}[tbhp]
	\centering
	\subfigure[Limit of $f_b$ against $\theta_2$ when $F=0.6$\label{fig:slack-angle}]{\includegraphics*[width=.49\textwidth]{./figure/plot/threshold_angle.pdf}}
	\subfigure[Limit of $f_b$ against $F$ when $\theta_2=15^{\circ}$\label{fig:slack-force}]{\includegraphics*[width=.49\textwidth]{./figure/plot/threshold_force.pdf}}
...
% 		また，実験時のフィンガの動きの例を図\ref{pic:example_slack}に示す．
	...
	\end{figure}
In Fig.\ref{fig:slack_conclusion}, "bias force" represents the value of $f_b$ in Eq.\ref{eq:independent-f} and this is bias force of tendon 2.
...
It is expected that nonelastic branching tendon mechanism become a notable factor on the field of robotic, especially anthropomorphic hand.
